...
class EquipmentAllocationController @Inject()(
  ...
  def listAllocationDetails: Action[AnyContent] = Action.async { request =>
    rbac.withRoles(request, InventoryView) { _ =>
      ...
      def parseInstantParam(name: String): Either[String, Instant] =
        request.getQueryString(name) match {
          case None => Left(s"Missing required query parameter '$name'")
          case Some(value) =>
            Try(Instant.parse(value)).toEither.left.map(_ => s"Invalid value for '$name'. Expected ISO-8601 instant, got: '$value'")
        }

      def parseIntParam(name: String, default: Int, min: Int = 1): Int =
        request.getQueryString(name)
          .flatMap(v => Try(v.toInt).toOption)
          .filter(_ >= min)
          .getOrElse(default)

      (request.getQueryString("start"), request.getQueryString("end")) match {

        case (Some(_), Some(_)) =>
          (parseInstantParam("start"), parseInstantParam("end")) match {
            ...
            case (Right(start), Right(end)) =>
              val page     = parseIntParam("page", 1)
              val pageSize = parseIntParam("pageSize", 20)
              val offset   = (page - 1) * pageSize

              for {
                total <- allocationRepo.countByAllocatedAtRange(start, end)
                details <- allocationRepo.listAllocationDetailsByAllocatedAtRange(start, end, offset, pageSize)
              } yield {
                val jsArray = details.map { d =>
                  JsObject(Seq(
                ...
                Ok(Json.obj(
                  ...
                  "pageSize"   -> pageSize,
                  "total"      -> total,
                  "totalPages" -> (if (pageSize <= 0) 1 else Math.max(1, Math.ceil(total.toDouble / pageSize).toInt)),
                  "start"      -> start.toString,
                  "end"        -> end.toString,
                  "items"      -> JsArray(jsArray.toVector)
                ))
              }
          }
  ...
  /**
   * List equipment allocations within a date range with pagination support.
   *
   * Date Range:
   *   - `start` (optional): ISO-8601 timestamp. Defaults to 180 days before now.
   *   - `end`   (optional): ISO-8601 timestamp. Defaults to the current time.
   *
   * If `start` or `end` are omitted, the default values are applied automatically.
   * If provided, both must be valid ISO-8601 instants.
   *
   * Pagination:
   ...
   *     - totalPages
   *     - start, end (final resolved date range)
   *     - items: array of allocation objects
   *
   * Requirements:
   *   - Requires `InventoryView` role to access.
   *
   * Possible responses:
   *   - 200 OK: successful filtered + paginated result
   *   - 400 BadRequest: invalid date format
   *   - 500 InternalServerError: unexpected failure
   */
  def listAllocationsByRange: Action[AnyContent] = Action.async { request =>
    rbac.withRoles(request, InventoryView) { _ =>
      import scala.util.Try

      val now = Instant.now()
      val defaultStart = now.minusSeconds(180L * 24 * 3600) // default 180 days range
      val defaultEnd   = now

      def parseInstantParam(name: String, defaultValue: Instant): Either[String, Instant] =
        request.getQueryString(name) match {
          case None =>
            Right(defaultValue)
          case Some(value) =>
            Try(Instant.parse(value)).toEither.left.map { _ =>
              s"Invalid value for '$name'. Expected ISO-8601 instant, got: '$value'"
            }
        }
      ...
      val startEither = parseInstantParam("start", defaultStart)
      val endEither   = parseInstantParam("end",   defaultEnd)

      (startEither, endEither) match {
        case (Left(err), _) =>
        ...
        case (Right(start), Right(end)) =>
          val page     = parseIntParam("page",     default = 1)
          val pageSize = parseIntParam("pageSize", default = 20)
          val offset   = (page - 1) * pageSize

          allocationRepo
            .countByAllocatedAtRange(start, end)
            .flatMap { total =>
              allocationRepo
                .listByAllocatedAtRange(start, end, offset, pageSize)
                .map { allocs =>
                  val itemsJsArray = JsArray(allocs.map(Json.toJson(_)).toVector)
                  ...
                  Ok(meta ++ Json.obj("items" -> itemsJsArray))
                }
            }
            .recover { case ex =>
              logger.error(s"Error listing allocations by range start=$start end=$end", ex)
              InternalServerError(Json.obj(
                "error"   -> "Failed to list allocations by range",
